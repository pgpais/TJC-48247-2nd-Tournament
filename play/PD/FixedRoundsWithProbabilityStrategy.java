...
public class FixedRoundsWithProbabilityStrategy extends Strategy {
    double cooperatePayoff;
    double defectPayoff;
    double defectToCooperatePayoff;
    ...
    public double[] setStrategy(int P, String[] labels, PlayStrategy myStrategy) {
        ...
        double probabilityForNextIteration = myStrategy.probabilityForNextIteration();
        double gainFromDefect = defectToCooperatePayoff - cooperatePayoff;
        double lossFromDefect = ((cooperatePayoff - defectPayoff) * probabilityForNextIteration) / (1-probabilityForNextIteration);
        double lossFromDefectIt = (cooperatePayoff - defectPayoff) * (iterationsLeft - 6);

        if(gainFromDefect > lossFromDefect || gainFromDefect > lossFromDefectIt){
